...
document.addEventListener('DOMContentLoaded',function(){
  document.querySelectorAll('.product-card:not(.product-card-disabled)').forEach(function(c,i){
    ...
    c.addEventListener('click',function(){
      if(c.dataset.ff){
        var sku=c.dataset.ff;
        var name=c.dataset.name;
        var price=parseInt(c.dataset.price)||0;
        openFfTopup(sku,name,price);
      } else {
        addToCart(c);
      }
    });
...
function openFfTopup(sku, name, price) {
  ffTopupSku = sku;
  ffTopupName = name;
  ffTopupPrice = price;
  document.getElementById('ffTopupTitle').textContent = '💎 ' + name;
  document.getElementById('ffTopupSubtitle').textContent = 'Harga: Rp ' + price.toLocaleString('id-ID');
  document.getElementById('ffUserId').value = '';
  document.getElementById('ffUserServer').value = '';
  var st = document.getElementById('ffTopupStatus');
  st.style.display = 'none';
  st.textContent = '';
  var btn = document.getElementById('ffTopupBtn');
  btn.disabled = false;
  btn.textContent = '⚡ Proses Top Up';
  var ov = document.getElementById('ffTopupOverlay');
  ov.style.display = 'flex';
}

function closeFfTopup() {
  document.getElementById('ffTopupOverlay').style.display = 'none';
}

function setFfStatus(msg, type) {
  var el = document.getElementById('ffTopupStatus');
...
async function submitFfTopup() {
  var userId = document.getElementById('ffUserId').value.trim();
  if (!userId || !/^\d{6,20}$/.test(userId)) {
    setFfStatus('⚠️ ID FF tidak valid, pastikan hanya angka!', 'err');
    return;
  }
  ...
  setFfStatus('🔄 Menghubungi server Digiflazz...', 'info');

  var refId = 'ALRECT-' + Date.now();
  ...
  try {
    ...
    if (data && data.data && data.data.status === 'Sukses') {
      setFfStatus('✅ Top up berhasil! Diamond masuk ke ID ' + userId, 'ok');
      btn.textContent = '✅ Berhasil!';
    } else if (data && data.data && data.data.status === 'Pending') {
      setFfStatus('⏳ Sedang diproses, tunggu beberapa menit ya!', 'info');
      btn.textContent = '⏳ Pending...';
    } else {
      var errMsg = (data && data.data && data.data.message) || (data && data.error) || 'Gagal, coba lagi';
      setFfStatus('❌ ' + errMsg, 'err');
      btn.disabled = false;
      btn.textContent = '⚡ Coba Lagi';
    }
  } catch (e) {
    setFfStatus('❌ Koneksi gagal, coba beberapa saat lagi', 'err');
    btn.disabled = false;
    btn.textContent = '⚡ Coba Lagi';
  }
}
